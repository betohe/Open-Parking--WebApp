angular.module( 'sample.takespace', [
'auth0'
])
.controller( 'TakeSpaceCtrl', function TakeSpaceController( $scope, auth, $routeParams, $http, zoneStorage, $location, store, mySocket  ) {
  console.log('taken!!');
      ...
      if(zones[i].id == $routeParams.id){
        ...
        zoneStorage.update(zones[i]).success(function(newZone) {
              if (newZone === 'null') { // Compare with a string because of https://github.com/angular/angular.js/issues/2973
                ...
                console.log(newZone);
                mySocket.emit('updatezone', {id: $routeParams.id, action:"enter"}); //op: enter or leave
              }
            }).error(function() {
              zone._saving = false;
